...
- 개별 예약 이율(`settlement_rate_override`) → 상품 기본 예약 이율(`default_settlement_rate`) → 일일 보상(`daily_reward`) 기준 순서로 fallback 정산합니다.
...
| `settle_due_staking_positions(p_limit)` | 만기 도래 포지션을 일괄 정산. 예약 이율이 없으면 `daily_reward` 기준으로 자동 fallback |
