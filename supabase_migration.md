...
각 함수 진입부의 `from("admins").select("id")` 를 `select("id, role")` 로 확장하고, **admin 계정 생성** 및 **기존 admins/agents 계정의 비밀번호·세션 조작** 은 호출자 `role === "super_admin"` 일 때만 허용하도록 강화했습니다. agent 생성/삭제 및 일반 사용자(user_profiles) 대상 작업(예: 일반 회원 비밀번호 리셋, 일반 회원 강제 로그아웃)은 모든 admin 이 그대로 수행 가능합니다.
...
#### (8) `backoffice_account_schema_sync_2026_05`

관리자/파트너 계정 생성 경로가 사용하는 컬럼을 명시 보강했습니다.

| 항목 | 변경 |
|---|---|
| `admins.email` | 관리자 생성 시 Supabase Auth 이메일과 관리자 목록 표시 이메일을 함께 저장 |
| `agents.grade` | 파트너 구분(`총판` / `대리점`) 저장 컬럼 |
| `agents.phone/email` | 파트너 연락처 및 로그인 이메일 저장 |
| `agents.bank_name/account/holder` | 파트너 출금 계좌 정보 저장. 은행명은 회원가입과 같은 은행 목록 드롭다운을 사용 |
| `agents.commission_balance` | 파트너 수익 잔고 수동 조정값 저장 |

적용 파일: `apps/user/supabase/migrations/033_backoffice_account_schema_sync_2026_05.sql`

#### (9) 백오피스 계정 생성 경로 및 가입코드 규칙

관리자/파트너 생성은 브라우저에서 Edge Function 을 직접 호출하지 않고 Next.js 서버 라우트 `POST /api/admin/backoffice/create` 를 통과합니다. 이 라우트가 `SUPABASE_SERVICE_ROLE_KEY` 로 Supabase Auth 사용자와 `admins`/`agents` 행을 함께 생성하므로 CORS/응답 파싱 실패로 인한 무반응을 줄이고, 실패 원인을 JSON 오류로 반환합니다.

| 항목 | 현재 규칙 |
|---|---|
| 관리자 생성 | `super_admin` 만 가능. `admins.email` 에 Auth 이메일 저장 |
| 파트너 생성 | 모든 admin 가능. `agents.grade` 는 `총판` 또는 `대리점` |
| 파트너 은행 | `apps/user/lib/constants/banks.ts` 의 공용 은행 목록만 선택 가능 |
| 파트너 가입코드 | 별도 입력이 없으면 `username` 을 대문자화하고 영문/숫자만 남긴 값 사용. 예: `awerwe` → `AWERWE` |
| 가입코드 중복 | 랜덤 suffix 를 붙이지 않음. 이미 사용 중이면 `409` 와 함께 다른 아이디 입력을 요구 |
| Edge Function | `admin-create-backoffice-account` 는 동일 로직의 백업 경로로 유지 (`verify_jwt: true`) |
